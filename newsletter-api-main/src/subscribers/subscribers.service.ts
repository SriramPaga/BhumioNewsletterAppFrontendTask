...
export class SubscriberService {
  ...
  async create(
    ...
    subscriber.customFields =
      createSubscriberDto.customFields;
    ...
    if (createSubscriberDto.organizationId) {
      const organization =
        await this.organizationRepository.findOne(
          {
            where: {
              id: createSubscriberDto.organizationId,
            },
          },
        );
      if (organization) {
    ...
    return this.subscriberRepository.save(
      subscriber,
    );
  }
    ...
    if (updateSubscriberDto.organizationId) {
      const organization =
        await this.organizationRepository.findOne(
          {
            where: {
              id: updateSubscriberDto.organizationId,
            },
          },
        );
      if (organization) {
